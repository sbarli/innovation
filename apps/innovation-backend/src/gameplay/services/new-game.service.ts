...
export class NewGameService {
  ...
  async validatePlayersExist(playerRefs: string[]): Promise<Player[]> {
    try {
      ...
      throw new Error(
        error?.message ?? 'gameplayService: Unable to validate players exist',
      );
    }
  }
  ...
  async validateUniqueGame(playerRefs: string[]): Promise<boolean> {
    try {
      ...
      throw new Error(
        error?.message ?? 'gameplayService: Unable to validate unique game',
      );
    }
  }

  getGameSetup(
    cardRefsByAge: CardRefsByAge,
    playerRefs: string[],
  ): TNewGameSetup {
    ...
    const { ageAchievements, deckMinusAchievements } =
      pickAgeAchievements(starterDeck);

    // select player starting hands (2 cards x n players) (remove from current deck)
    const { playerStarterHands, deckMinusStarterHands } =
      selectStarterHandsForPlayers(deckMinusAchievements, playerRefs);
